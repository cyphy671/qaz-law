...
class SearchActMetadata(BaseModel):
    ...
    approval_place: str = Field(
        default="",
        alias="approvalPlace",
        description="Place of approval",
        examples=["100051000000"],
    )
    ...
    action_date: date | None = Field(
        None, alias="actionDate", description="Action date"
    )
    judiciary_doc_number: str | None = Field(
        None, alias="judiciaryDocNumber", description="Judiciary document number"
    )
...
class SearchPage(BaseModel):
    ...
    documents_found: int = Field(
        ..., alias="documentsFound", description="Total number of documents found"
    )
    documents: list[SearchActMetadata] = Field(
        ..., alias="list", description="List of documents"
    )
...
class ActMetadata(BaseModel):
    ...
    judiciary_approval_date: date | None = Field(
        None, alias="judiciaryApprovalDate", description="Judiciary approval date"
    )
    effective_date: date | None = Field(
        None, alias="effectiveDate", description="Effective date"
    )
...
class ChangeCause(BaseModel):
    ...
    requisites: BilingualText = Field(
        ..., description="Requisites of the causing document"
    )
...
class ContentElement(BaseModel):
    ...
    change_id: str | None = Field(
        None, alias="changeId", description="Change ID of the element"
    )
...
class Document(BaseModel):
    ...
    version_date: date = Field(..., alias="versionDate", description="Version date")
    ...
    content: list[dict] | str = Field(
        default=None,
        description="Document content (either as element list or as HTML string)",
    )
    initial_publication_date: FlexDate | None = Field(
        None, alias="initialPublicationDate", description="Initial publication date"
    )
    pages_count: int = Field(
        ..., alias="pagesCount", description="Total number of pages"
    )
    index: list[dict] | None = Field(default=None, description="Document index")
    temporary_revoked: bool | None = Field(
        None,
        alias="temporaryRevoked",
        description="Whether document was revoked temporarily",
    )

    @model_validator(mode="after")
    def verify_data(self) -> Self:
        # metadata.registry_number also contains ID, but it differs sometimes, and we don't store it
        assert self.id == self.code, f"Document ID diverges: {self.id}/{self.code}"
        return self
...
class MultiLangDocument(BaseModel):
    """A legal document with multiple versions in multiple languages."""

    rus: Document | None = Field(None, description="Document in Russian")
    kaz: Document | None = Field(None, description="Document in Kazakh")

    @property
    def docs(self):
        return [doc for doc in [self.rus, self.kaz] if doc]
    ...
    def initial_publication_date(self):
        return (
            self.rus.initial_publication_date
            if self.rus
            else self.kaz.initial_publication_date
        )

    @property
    def actual_version(self):
        return self.rus.actual_version if self.rus else self.kaz.actual_version

    @property
    def version_date(self):
        return self.rus.version_date if self.rus else self.kaz.version_date

    @property
    def version(self):
        return self.rus.version if self.rus else self.kaz.version
